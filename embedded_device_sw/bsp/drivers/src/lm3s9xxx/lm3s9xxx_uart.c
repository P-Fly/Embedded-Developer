				   ...
				   const uart_config_t *config)
{
	...
	/* Configure the UART */
	MAP_UARTConfigSetExpClk(handle->uart_base,
				MAP_SysCtlClockGet(),
				baudrate,
				(word_length | stop_bits | parity));

	/* Configure FIFO Level Select */
	MAP_UARTFIFOLevelSet(handle->uart_base, UART_FIFO_TX7_8,
			     UART_FIFO_RX7_8);
...
#endif
#if defined(CONFIG_UART0_TX_RING_BUFF_SIZE) || \
	defined(CONFIG_UART0_RX_RING_BUFF_SIZE)
	MAP_IntPrioritySet(handle->uart_interrupt,
