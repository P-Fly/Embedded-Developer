 ...
 */
static int stm32wbxx_uart_configure(const object* obj, const uart_config_t* config)
{
...
#ifdef CONFIG_UART1_TX_RING_BUFF_SIZE
    ret = ring_buffer_init(&handle->tx, uart1_tx_ring_buff, sizeof(uart1_tx_ring_buff));
    if (ret)
        return ret;
#endif

#ifdef CONFIG_UART1_RX_RING_BUFF_SIZE
    ret = ring_buffer_init(&handle->rx, uart1_rx_ring_buff, sizeof(uart1_rx_ring_buff));
    if (ret)
        return ret;
#endif
